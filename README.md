...
- **Consumer Group Offset Persistence** Implements checkpointing. Consumers commit progress to broker which then persists to disk so processing resumes seamlessly across system restarts
...
## O(1) Message Retrieval Logic

1. Lookup `targetByte = topicOffsetMap[topic][N]` ($O(1)$ memory lookup)
2. Lookup `nextByte = topicOffsetMap[topic][N+1]` (or current `file.Size()` if reading the tail).
